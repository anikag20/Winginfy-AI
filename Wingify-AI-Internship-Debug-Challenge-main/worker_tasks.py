...
@shared_task
def test_task():
    print("✅ Dummy task received and running")
    return "Success"

@celery_app.task(bind=True, max_retries=2, default_retry_delay=60)
def process_blood_test_analysis(self, encrypted_file_data: str, query: str):
    print("📥 Args received in Celery task")
    print(f"📊 encrypted_file_data type: {type(encrypted_file_data)}")
    print(f"📊 query type: {type(query)}")

    print("📣 Celery task received")
